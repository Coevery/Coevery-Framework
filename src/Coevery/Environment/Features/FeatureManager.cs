...
namespace Coevery.Environment.Features {
			        ...
			        (currentFeatureId, featuresState) => {
				        ...
				        return feature.Key.Dependencies
				                      .Select(fId => {
					                      ...
					                      if (states.Count == 0) {
											  throw new CoeveryException(T("Failed to get state for feature {0}", fId));
					                      }

					                      if (states.Count > 1) {
											  throw new CoeveryException(T("Found {0} states for feature {1}", states.Count, fId));
					                      }
